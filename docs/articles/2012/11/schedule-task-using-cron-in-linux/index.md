...
created: '2012-11-27T11:28:08-08:00'
...
[AirFlow](job-scheduling-and-management-using-apache-airflow)
.
...
1. The information of scheuled tasks are saved in the file `/etc/crontab`.
...
1. As long as you use the `crontab` commands to edit the file `/etc/crontab`,
...
1. If your cron job consists of multiple shell commands,
...
1. You can schedule a frequently run task using crontab
...
1. Run the command `rsnapshot daily` daily at 22:00.

   ```
    0   22  *     *     *     rsnapshot daily
   ```

1. Run the command `rsnapshot daily` hourly at the 5th minutes,
...
1. Run a command at 22:00 on the 1st day of every month.

   ```
    0   22  1     *     *     command_to_run
   ```

1. There is no intrinsic way to run commands weekly using cron.
