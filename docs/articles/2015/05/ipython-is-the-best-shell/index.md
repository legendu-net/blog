...
1. If you do want to use a Unix/Linux shell,
...
1. According to [Configuration file location](https://ipython.readthedocs.io/en/stable/development/config.html#configuration-file-location),
...
1. You can use `?%alias_or_magic` to check the definition of an alias or line magic.
   Notice that `%` is required for aliases.
   The suffixing `?` doesn't work in this case.

1. Wild cards can be used to match methods in dynamic object retrospection!
...
1. Python variables can used in a shell command.
   For example,
...
1. When you use the prefix `!` to run a shell command,
...
1. `%edit` is very useful for editing the definition of Python objects (functions, classes, etc.).

1. `%notebook` exports the current IPython history to a notebook file.
   For example,
   to export the history to `foo.ipynb` do `%notebook foo.ipynb`.

1. `%env` shows and set environment variables.
   Of course,
...
1. `%run` runs a Python script or a Jupyter/Lab notebook
...
1. `%load` inserts the content of a text file into the current cell.
   The `%load` statement itself is commented out after loading the content of the text file.

1. `%%writefile` writes the content of the current cell to an external file.
...
1. You can enable auto call (calling without using parentheses) of Python functions/method
...
1. If you parsing arguments in an IPython script,
...
1. IPython automatically beautify outputs.
