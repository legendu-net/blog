...
1. If you have multiple versions of Spark installed,
...
1. PySpark does not support converting `$"col"` to a Column implicitly.
   However,
   the function `pyspark.sql.functions.col` works the same as in Spark.

1. [Pandas UDFs](https://spark.apache.org/docs/latest/sql-pyspark-pandas-with-arrow.html#pandas-udfs-aka-vectorized-udfs)
   is preferred to UDFs.
   Please refer to
   [User-defined Function (UDF) in PySpark](user-defined-function-udf-in-pyspark)
   for more discussions.

1. [pyspark-stubs](https://github.com/zero323/pyspark-stubs)
...
1. When working with relatively large data in a local version of Spark in Jupyter/Lab notebook,
...
1. The command-line option `--jars` is equivalent to `spark.jars` when you use `--conf`.
...
1. If you want to leverage a Spark cluster in a Jupyter/Lab notebook,
...
1. When you run Spark or PySpark in a Jupyter/Lab notebook,
...
1. [Build a portable Python environment using python-build-standalone](packaging-python-dependencies-for-pyspark-using-python-build-standalone)
1. [Build a portable Python environment using conda-pack](packaging-python-dependencies-for-pyspark-using-conda-pack)
1. [Packaging Python dependencies using pex](packaging-python-dependencies-for-pyspark-using-pex)
