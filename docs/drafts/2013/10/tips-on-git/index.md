...
1. In an enterprise environemtn,
...
1. Do not work on the master branch directly.
...
1. Keep each commit small and specific.
...
1. Git support several protocols: Git, SSH and HTTP.
...
1. it seems to me that you should use many smaller files
   rather than a single big file to avoid conflicts while working collaboratively.

1. not a good practice to track non text files
...
1. It seems that the git repo for criss's project is very large even though it is the first commit.
...
1. Git with http proctol ask for passwords when one push to or pull from a repository.
   There are ways to cache Git password in Windows,
   however,
   I'd rather save the pain and use AutoHotkey to help me type in password.

1. To rebase, follow the steps below.
...
1. Use the following comamnd to clone into a specific directory.
   Please refer to
...
1. Overwrite the remote repository `origin` using the master branch.
...
1. Tells git default branches to use when pushing
...
1. Add all modifed tracked files for commit
...
1. Remove a file but keep it in the local repository

   ```
    git rm --cached file_name
   ```

1. Init current directory as a bare repository (for local sharing purpose).
...
1. Checkout the last branch that you were on.

   ```
    git checkout -
   ```

1. Check out the remote branch named "dev".
...
1. Create a branch named "dev" and switch to it.

   ```
    git checkout -b dev
   ```

1. Merge the branch named "hotfix" to the current branch

   ```
    git merge hotfix
   ```

1. Rename "old_branch" to "new_branch".
...
1. Force to delete a branch no matter it has been merged or not.
...
1. `git log origin/master..HEAD` lists unpushed commits.
   Uncommitted changes are not listed.
   Notice the syntax `git log <since>..<until>` is similar to Bash `..` in list.

1. Switching branches carries uncommitted changes with you.
...
1. Push tags to remote repository

   ```
    git push origin v0.1
   ```

1. Create a branch at a specific tag
...
1. Git in Windows automatically convert Linux/OSX line termiantors to Windows line terminators.
...
1. Git track permissions of files by default.
...
1. Git does not colorize output automatically on some (old) Linux servers.
   You can use `git config --global color.ui auto` to force Git to colorize output.

1. List all tracked files in a git repository
