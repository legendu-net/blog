...
1. When you create a table by selecting records from another table,
...
1. Outer join is preferred over inner join if you doing some exploring analyses
...
1. `window_function() over (partition by col1, col2 order by col3)`
...
1. `row_number() over ()` It is allowed to specify no partition for window functions.
   In that case, all records is treated as one partition.

1. When using integer constant in SQL code, prefer BigInt, i.e., suffix them with `L`.
   This can save you troubes of overflow if count/sum a large table.

1. Always assume there are data issues (such as missing values, duplicates, etc.) unless forced by table constraints
   as this is often the fact.

1. If you write some database-based application,
...
1. Good to keep all kind of IDs even if you won't need them currently,
   you nev.. know when you are going to need them ...

1. Avoid using subqueries in SQL.
...
1. analytical functions are powerful
   row_number() over partition
   another way is to use self join

1. `select count(*) from table t;`
...
1. refresh in SQLDeveloper to show newly created talbes

1. You must put all columns of the SELECT
...
1. the select distinct, distinct describes record, not column.
...
1. It is always good practice to indicate the version of SQL in your sql code ....
...
1. case when, break at first match, no need to manually break (unlike C/C++)

1. avoid using keywords as column names,
   if you do want use a keyword as a column name,
   you have to double quote it (e.g., "as").
   can also use [], '', in MS SQL, etc.

1. always specify (unique) primary index when you create a table

1. if not group by statement, then all records as 1 group
   if there is a aggregation function.

1. be careful when you use the
...
1. Even if observations in a table is sorted,
...
1. extra `;` can be really dangerous ...
