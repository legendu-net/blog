...
created: '2015-05-02T21:55:44-07:00'
...
1. Be careful when you use integers as column names or indexes
...
1. `pandas` keeps the underlying precision (instead of the display precision)
   while reading Excels files.
   However,
   be aware that pandas displays 7 significant digits by default.

1. Label-based slicing is inclusive
   as it is not clear what "pass by 1" means for label-based slicing.

1. You can apply a function on a row/column using the method `DataFrame.apply`.
   However,
...
1. it is strange that sometimes series of booleans cannot be used for slicing?

1. what operations cause a data frame to sort its columns and/or rows?

1. difference between merge and join?

1. it seems that DataFrame.str.replace and Series.str.replace
