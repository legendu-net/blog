...
created: '2017-01-05T23:55:46-08:00'
...
1. It is almost always a good idea to filter out null value in the joinining columns before joining
...
1. When you join 2 tables and apply filtering conditions on those 2 tables,
...
1. It is always a good idea to check the execution plan of your Spark job
   to make sure that things are as expected
   before you actually run it.

1. Before you use a HDFS table,
...
1. When you select columns from a table (or from joining of multiple tables),
...
1. It is suggested that you always call `spark.stop()`
...
1. Best to use the JVM option `-XX:MaxDirectMemorySize` to limit the maximum directory memory used.
...
1. If you are sure that your Spark application is production ready,
...
1. `spark-shell` accepts `--queue` (for specifying the queue to submit jobs) as parameter!
