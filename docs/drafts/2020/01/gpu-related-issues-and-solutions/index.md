...
created: '2020-01-03T09:22:43-08:00'
...
1. torch.cuda.empty_cache() doesn't help if memory is not enough

1. It is suggested that you train deep learning models on Linux
   (even if you can also do it on Windows).

1. If neural network's performance is not improving during training,
   you can try increasing batch size or reduce learning rate.

1. 8G GPU memory is too small for training large deep learning models.
...
1. Switch to TTY (using the shortcut Ctrl + Shift + F1, F2, etc.) if you are using Linux,
   which can already help free most used GPU memories.

1. Boot your Linux machine into text model if you absolutely want squeeze all GPU memory
...
1. clear PyTorch caches

1. add `torch.cuda.empty_cache()` before training each epoch

1. Free GPU memory from other applications (see the previous section)

1. [Reduce Memory Needed to Train Deep Learning Models](reduce-memory-needed-to-train-deep-learning-models)
