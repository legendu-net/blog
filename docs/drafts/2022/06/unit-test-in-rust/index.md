...
1. [debug_assert!](https://doc.rust-lang.org/std/macro.debug_assert.html)
   is similar to
...
1. [static_assertions](https://crates.io/crates/static_assertions)

1. All assert macros support an optional error message on failure.
...
1. Refactor your unit test functions to take parameters using other ways
   instead of command-line arguments.
   For example,
   you make your unit test functions to read configuration/data files
   from a specific location.

1. Make the test actually execute a non-cargo-test binary that is for exactly this purpose only.
