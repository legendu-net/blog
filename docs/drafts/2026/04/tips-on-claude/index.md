...
- Type in `@` within a live claude session brings up a dropdown list for you to select files.
  ...
  you can press `ctrl + g` to edit the prompt using Neovim
  (assume you have the environment variable `$EDITOR` configured to `nvim`),
  which allows you to fully leverage the power of Neovim.
  For example,
  you can read in file names under the current directory using `:r !ls`.
