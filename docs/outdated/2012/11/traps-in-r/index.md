---
title: Traps in R
created: '2012-11-09T23:47:21-08:00'
...
1. [Zero-Length Vector Issue in R](zero-length-vector-issue-in-r)
...
1. Always be careful when there is an operation based on two vectors.

1. Sequence operator (`:`) has a higher priority over arithmetic operators (`+`, `-`, `*`, `/`),
   but a lower priority than the power operator (`^`) and element operator (`[]`).

1. Expression `2^1^2` is calculated from right to left in R.

1. Avoid use function `median` if you can,
...
1. For functions which takes `...` as one of its parameters,
...
1. To get the length of a vector,
   you can use `length`.
   However,
   to get the length of a string,
   you should use `nchar`.

1. `round` in use the close even number strategy which might not what you expect,
...
1. It is often that one want to build a larger array based on small arrays.
   For example,
...
1. R runs commands in `Rprofile.site`,
...
1. Usually the number of levels of a factor equals the number of distinct elements in the factor,
   however,
