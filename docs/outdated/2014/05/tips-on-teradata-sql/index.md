...
created: '2014-05-15T12:10:03-07:00'
date: '2026-06-12T22:15:55-07:00'
authors:
  - bendu
label: tips-on-teradata-sql
license: CC-BY-4.0
tags:
  - programming
  - tips
...
Please refer to
[Syntax of Teradata SQL](syntax-of-teradata-sql)
.

## Performance Tips

[Performance](general-tips-on-sql-performance)

## Trick and Trap

1. Must use on commit preserve rows to persist data if you create a volatile table.

1. Data cast using parentheses is discouraged in Teradata SQL.
   Use the `CAST` function instead.
   Another way is to use suffix to indicate the type of literal values,
   e.g., `'1'XI8`.

1. Below code get informtion about the table `cat123`.
...
1. You cannot create a new table using `insert into`.
...
1. insert one by one works well however insert into using select seems not to work as expected ...
...
1. You can use `sel` as short for `select` in Teradata SQL,
   however,
...
1. Use `--` for one line comment and `/*...*/` for multiple line comment.
   The `/*...*/` way is recommended as it is a widely used way for commenting.

1. The `as` keyword is optional when creating column aliases,
   however,
...
1. `union (all)` does not match column names.
...
1. Both count and sum can be used to count the number of rows satisfying some condition.
...
1. Do not use natural joins (relying on same column names for joining) as it is dangerous to do so.

1. When joining multiple tables,
...
1. Displays the code used to generate the view or table.

   ```
    show table table_name;
   ```

1. Display table schema (i.e., show column names and attributes).
...
1. Error code 3754: precision error, character, numeric, .. -> float ...
