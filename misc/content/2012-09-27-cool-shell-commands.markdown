...
2. `apropos` searches description of commands 
...
4. `!n` (where $n$ is a natural number) stands for the $n^{th}$ command in the command history.

4. `!-n` stands for the last but $n-1$ command in the command history, 

3. `!foo` stands for the most recent command starting with `foo`.

### Manipulating Commands

0. You can use the suffix `:-` to get rid of 
...
You can preview (rather than run) a retrived and manipulated (using string substitution) command by adding the suffix to the command.  

5. In the above commands, 
