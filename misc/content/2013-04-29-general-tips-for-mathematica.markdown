...
Status: published
Title: General Tips for Mathematica
Date: 2015-05-19 01:42:01
...
calculate $sin 5 + cos 5$, you can use command
        ...
        `Sin[x]+Cos[x]/.x->5` instead of using
        `x=5;Sin[x]+Cos[x]`.
...
8. Function `ReadList` can read data from a file into a list of
specified patterns.
...
result when you use these two functions.
    ...
    dataOm2030=Import["D:\\Dropbox\\Research\\thesis\\simulation\\MathOptim\\mcmc_
    20_30\\rms_20_30_1.bin", "real64"];
...
7. `Tuples[list,n]` generates a list of all possible n-tuples of
elements from list.

8. `Position[expr,pattern]` (similar to function `which` in R)gives a
list of the positions at which objects matching pattern appear in
`expr`.
...
11. Function `Join` can join vectors, matrices, arrays or lists together
in a specified way.

## Graphics and Animation

1. Mathematica is very powerful at plotting graphics and presenting
animations. It can do almost everything that you can think about.
Just give it a try if you come across a hard situation in other
software.
...
1. In Mathematica you use comma to separate different parts of a `For`
loop, which is different from most other programming languages.
...
3. You can insert object(s) in mathematica workspace into the email
body directly when you use `SendMail` to send emails, which is very
convenient if you want to send the running results of mathmatica via
email.
