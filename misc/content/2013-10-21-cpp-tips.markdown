...
0. Vim with the YouCompleteMe and UltiSnips plugins is a sweet choice for C++ IDE.
...
28. the error of no convertion from `T *` to `T`, 
...
6. If the definition of a class X does not explicitly declare a move constructor, 
...
34. about libraries for math special functions and statistical distributions:
...
13. The `operator<<` method of `stream` is for writing formated text 
...
9. subtraction between unsigned integers are well defined, 
...
3. Do not declare objects const if want to move from them.
